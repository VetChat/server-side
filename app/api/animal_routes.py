...
from ..schemas import AnimalRead, AnimalCreate
...
async def add_animal(request: Request, animal: AnimalCreate, db: Session = Depends(get_db)) -> AnimalRead:
    ...
    if existing_animal:
        raise HTTPException(status_code=400, detail=f"Animal with name {animal.name} already exists")
    
    animal_data = animal_crud.create_animal(animal.name)
