...
@router.get("/symptoms/animal/{animal_id}/not-in-list", response_model=List[SymptomRead], tags=["Symptoms"])
@limiter.limit("10/minute")
async def get_symptoms_not_in_list_by_animal_id(request: Request, animal_id: int, db: Session = Depends(get_db)) -> (
        List)[SymptomRead]:
    # Retrieve the ticket to get the animal_id
    animal_crud = AnimalCRUD(db)
    animal = animal_crud.fetch_animal_by_id(animal_id)
    ...
    # Get list of symptoms by animal_id
    symptom_crud = SymptomCRUD(db)
    symptoms_data = symptom_crud.fetch_symptoms_not_in_list_by_animal_id(animal.animal_id)
    symptoms_response: List[SymptomRead] = [
        SymptomRead(
            symptomId=symptoms.symptom_id,
            symptomName=symptoms.symptom_name,
        )
